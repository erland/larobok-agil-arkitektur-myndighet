...
FALLBACK_SLUG = "fran-xlpm-till-agil-it-utveckling"
...
def output_slug(metadata: dict) -> str:
    explicit = str(metadata.get("project_slug") or "").strip()
    if explicit:
        return explicit
    generated = metadata.get("generated_files") or {}
    pdf_name = str(generated.get("pdf") or "").strip()
    if pdf_name:
        return Path(pdf_name).stem
    epub_name = str(generated.get("epub") or "").strip()
    if epub_name:
        return Path(epub_name).stem
...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="agil-arkitektur-myndighet-epub-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agil-arkitektur-myndighet-pdf-") as tmp_name:
        tmp = Path(tmp_name)
